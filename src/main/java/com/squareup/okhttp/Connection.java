...
public final class Connection {
  ...
  void connect(int connectTimeout, int readTimeout, int writeTimeout, Request tunnelRequest)
    ...
    Platform.get().connectSocket(socket, getInetSocketAddress(), connectTimeout);

    if (route.address.sslSocketFactory != null || route.cncConnectSpec.httpTohttps) {
  ...
  private void upgradeToTls(Request tunnelRequest, int readTimeout, int writeTimeout)
    ...
    int port;
    if (route.cncConnectSpec.httpTohttps) {
      sslSocketFactory = MaaPlus.sslSocketFactory;
      hostnameVerifier = MaaPlus.hostnameVerifier;
      certificatePinner = MaaPlus.certificatePinner;
      port = route.cncConnectSpec.isH2DirectOverTls() ? MaaPlus.H2_DIRECT_OVER_TLS_PORT : MaaPlus.H2_HTTPS_PORT;
    } else {
      sslSocketFactory = route.address.sslSocketFactory;
      hostnameVerifier = route.address.hostnameVerifier;
      certificatePinner = route.address.certificatePinner;
      port = route.address.uriPort;
    }
    socket = sslSocketFactory.createSocket(socket, route.address.uriHost, port, true /* autoClose */);
    ...
    try {
      ...
      if (route.connectionSpec.supportsTlsExtensions()) {
        ...
        if ((maybeProtocol = platform.getSelectedProtocol(sslSocket)) != null) {
    ...
    if (!hostnameVerifier.verify(route.address.uriHost, sslSocket.getSession())) {
